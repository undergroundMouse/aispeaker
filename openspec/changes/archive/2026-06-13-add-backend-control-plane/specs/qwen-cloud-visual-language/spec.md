...
**Reason**: Gateway telemetry authority moves to the backend control plane; the client-side `GatewayCloudVisualLanguageProvider` wrapper is no longer the production enforcement point.
**Migration**: Record telemetry in the server gateway during `POST /api/v1/cloud/visual-answer` and return telemetry metadata in the API response for client display.

...
export class MockLocalVisionAnalyzer implements LocalVisionAnalyzer {
  ...
  async analyze({ frame, transcript, language }: LocalVisionAnalysisInput): Promise<LocalVisionSignals> {
    const normalized = normalizeVisionPhrase(transcript)
...
export function normalizeVisionPhrase(input: string): string {
  return input
    .trim()
    .toLocaleLowerCase()
    .replace(/[，。！？,.!?]/g, '')
...
export function getTopCandidate(candidates: VisionCandidate[], threshold = 0): VisionCandidate | null {
  return (
    candidates
